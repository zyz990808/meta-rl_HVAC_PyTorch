...
def ddpg_torch(
    ...
    for t in range(steps):
        ...
        o = o2
        ep_return += r
        ep_exceedance += compute_step_exceedance(o2, env)
        ...
        if buffer.size > update_after:
            ...
            with torch.no_grad():
                ...
                target_q = batch["rew"] + gamma * (1 - batch["done"]) * target_critic(
                    batch["next_obs"],
                    next_a
                )
            ...
            # ----- actor update only after warm-up -----
            if t > warmup_steps:
                ...
                for p, old_p in zip(actor.parameters(), old_actor.parameters()):
                    p.data = old_p.data + actor_step_scale * (p.data - old_p.data)
        ...
        if d:
            print(
                f"[DDPG] Episode done | Return: {ep_return:.2f} | "
    ...
    if len(last_records) > 0:
        last_best_record = max(last_records, key=lambda x: x["return"])
        last_min_exceed_record = min(last_records, key=lambda x: x["exceedance"])
